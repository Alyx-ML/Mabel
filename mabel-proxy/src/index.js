...
async function pipeGemmaAttempt(controller, encoder, env, messages) {
  const firstTextDeadline = Date.now() + MODEL_FIRST_TEXT_TIMEOUT_MS;
  let reader;
  let pending = "";
  ...
  try {
    const stream = await withTimeout(
      ...
      firstTextDeadline - Date.now(),
      "Gemma inference did not start in time"
    );
    reader = stream.getReader();
    const decoder = new TextDecoder();
    while (true) {
      const readTimeout = textStarted
        ? MODEL_STREAM_IDLE_TIMEOUT_MS
        : firstTextDeadline - Date.now();
      const { value, done } = await withTimeout(
        reader.read(),
        readTimeout,
        textStarted ? "Gemma stream stopped responding" : "Gemma produced no text in time"
      );
      ...
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        try {
          ...
          console.error(JSON.stringify({
            route: "/chat",
            event: attempt === 1 && !error.textStarted ? "model_retry" : "model_failed",
            message: String(error?.message || "Unknown inference failure").slice(0, 160)
          }));
          if (attempt === 1 && !error.textStarted) {
            controller.enqueue(encoder.encode(": retrying same Gemma model\n\n"));
            continue;
          }
